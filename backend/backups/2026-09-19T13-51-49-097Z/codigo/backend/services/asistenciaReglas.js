...
function calcularMarcacion(body, programacion, anterior = null) {
  ...
  if (ingreso && salida) {
    ...
    if (duracion <= 0) {
      if (!nocturno || duracion === 0) throw error('La salida debe ser posterior al ingreso; para cruzar medianoche asigne un horario nocturno.');
      duracion += 1440;
    }
    horas = Math.round(duracion / 60 * 100) / 100;
  }
  if (ingreso && ['presente', 'tardanza'].includes(estado)) {
    ...
    // Los registros previos a las instantáneas de tolerancia conservan el cálculo si no cambió el ingreso.
    if (anterior && anterior.tolerancia_minutos == null && String(anterior.hora_ingreso || '').slice(0, 5) === ingreso.slice(0, 5) && ['presente', 'tardanza'].includes(anterior.estado)) tardanza = Number(anterior.minutos_tardanza || 0);
