...
(async () => {
  ...
  const out = path.resolve(__dirname, offline ? '../backups/validacion-offline' : '../backups/validacion-visual');
  ...
  try {
    ...
    assert.equal(external.length, 0, 'Los documentos no deben depender de Internet.');
    await fs.writeFile(path.join(out, 'interacciones.json'), JSON.stringify({ checkedAt: new Date().toISOString(), internetBlocked: offline, externalRequests: external, results, pdfResponses, javascript: errors }, null, 2));
