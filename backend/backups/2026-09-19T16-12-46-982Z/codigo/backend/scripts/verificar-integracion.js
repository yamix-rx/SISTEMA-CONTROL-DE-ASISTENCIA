...
async function main(){
  ...
  const fechaRelativa=dias=>new Date(new Date(require('../services/asistenciaReglas').hoy()+'T12:00:00Z').getTime()+dias*86400000).toISOString().slice(0,10);
  const vencimiento=fechaRelativa(15);
  const payload={numero_documento:(BigInt(stamp)+1n).toString(),nombres:'Persona María',apellidos:'Prueba Álvarez',empresa_id:ids.empresaId,area_id:ids.areaId,cargo_id:ids.cargoId,puesto:'Asistente de gestión',fecha_vencimiento_convenio:vencimiento,fecha_ingreso:'2026-01-01',tipo_vinculo:'practicante preprofesional',estado:'activo',horas_totales_asignadas:16,observaciones_rrhh:'NOTA INTERNA: solo Recursos Humanos'};
  ...
  const rrhh=(await request('/personal',{body:{...payload,numero_documento:(BigInt(stamp)+2n).toString(),nombres:'Responsable',apellidos:'Pruebas RRHH',tipo_vinculo:'trabajador',fecha_vencimiento_convenio:null},status:201})).id;
  ...
  const fichaInicial=(await request(`/personal/${empleadoId}`)).data.empleado;
  assert.equal(fichaInicial.puesto,payload.puesto);assert.equal(fichaInicial.fecha_vencimiento_convenio,vencimiento);
  const convenios=async()=> (await request('/dashboard?empresa_id='+ids.empresaId,{rol:'rrhh'})).conveniosPorVencer;
  assert.ok((await convenios()).some(x=>Number(x.empleado_id)===empleadoId));
  const [[sinContrato]]=await pool.query('SELECT COUNT(*) total FROM contratos WHERE empleado_id=?',[empleadoId]);assert.equal(sinContrato.total,0);
  await request(`/personal/${empleadoId}`,{method:'PUT',body:{...payload,fecha_vencimiento_convenio:fechaRelativa(45)}});
  assert.ok(!(await convenios()).some(x=>Number(x.empleado_id)===empleadoId));
  const {puesto,fecha_vencimiento_convenio,...sinCamposNuevos}=payload;
  await request(`/personal/${empleadoId}`,{method:'PUT',body:sinCamposNuevos});
  const conservada=(await request(`/personal/${empleadoId}`)).data.empleado;
  assert.equal(conservada.puesto,puesto);assert.equal(conservada.fecha_vencimiento_convenio,fechaRelativa(45));
  await request(`/personal/${empleadoId}`,{method:'PUT',body:{...payload,puesto:'',fecha_vencimiento_convenio:null}});
  const vacia=(await request(`/personal/${empleadoId}`)).data.empleado;
  assert.equal(vacia.puesto,null);assert.equal(vacia.fecha_vencimiento_convenio,null);
  await request(`/personal/${empleadoId}`,{method:'PUT',body:{...payload,fecha_vencimiento_convenio:'2026-02-30'},status:400});
  await request(`/personal/${empleadoId}`,{method:'PUT',body:payload});
  const propios=(await request('/mi-panel',{rol:'colaborador'})).data.empleado;
  assert.equal(propios.puesto,puesto);assert.equal(propios.fecha_vencimiento_convenio,vencimiento);
  const practicantes=(await request('/personal?tipo_vinculo=practicante&empresa_id='+ids.empresaId)).data;
  assert.ok(practicantes.some(x=>Number(x.id)===empleadoId));assert.ok(!practicantes.some(x=>Number(x.id)===rrhh));
  const contratosCatalogo=await request('/contratos/catalogos');
  assert.ok((contratosCatalogo.tipos_contrato||contratosCatalogo.data?.tipos_contrato).includes('Convenio de prácticas'));
  await request(`/personal/${empleadoId}`,{method:'PUT',body:{...sinCamposNuevos,tipo_vinculo:'trabajador',horas_totales_asignadas:12}});
  const trabajador=(await request(`/personal/${empleadoId}`)).data;
  assert.equal(Number(trabajador.empleado.horas_totales_asignadas),12);assert.equal(trabajador.progresoHoras.esPracticante,false);
  assert.equal(trabajador.empleado.fecha_vencimiento_convenio,vencimiento);
  const [[historialPracticas]]=await pool.query('SELECT horas_meta FROM practicante_detalles WHERE empleado_id=?',[empleadoId]);
  assert.equal(Number(historialPracticas.horas_meta),16);
  assert.ok(!(await convenios()).some(x=>Number(x.empleado_id)===empleadoId));
  await request(`/personal/${empleadoId}`,{method:'PUT',body:payload});
  log('puesto separado, convenio editable y alerta independiente de contratos');
  ...
  // Las filas antiguas no permiten deducir su horario original del horario actual.
  const legacyConnection=await mysql.createConnection(options());
  let legacyId;
  try {
    for(const fecha of ['2026-09-17','2026-09-18']) {
      const [result]=await legacyConnection.query(`INSERT INTO asistencias
        (empleado_id,fecha,hora_ingreso,hora_salida,minutos_tardanza,horas_trabajadas,estado,registrado_por_usuario_id)
        VALUES (?,?,'08:17','16:17',17,8,'tardanza',?)`,[rrhh,fecha,ids.adminId]);
      if(fecha==='2026-09-17')legacyId=result.insertId;
    }
  } finally {await legacyConnection.end();}
  const legacyBody={empleado_id:rrhh,fecha:'2026-09-17',hora_ingreso:'08:10',hora_salida:'16:10',estado:'presente',observacion:'Corrección verificada del registro antiguo'};
  await request('/asistencias/marcar',{body:legacyBody,status:400});
  const confirmacion={confirmada:true,hora_entrada:'08:00',hora_salida:'17:00',tolerancia_minutos:0,cruza_medianoche:false,motivo:'Horario original contrastado con el registro de prueba'};
  const legacyCorregida=await request('/asistencias/marcar',{body:{...legacyBody,programacion_historica:confirmacion}});
  assert.equal(legacyCorregida.data.minutosTardanza,10);assert.equal(legacyCorregida.data.horasTrabajadas,8);
  assert.equal(legacyCorregida.data.hora_programada_entrada,'08:00:00');
  await request('/asistencias/marcar',{body:{...legacyBody,programacion_historica:{...confirmacion,hora_entrada:'09:00'}},status:409});
  const auditoriaLegacy=(await request('/auditoria?tabla=asistencias')).registros.filter(a=>Number(a.registro_id)===legacyId&&a.accion==='UPDATE');
  assert.equal(auditoriaLegacy.length,1);
  assert.equal(auditoriaLegacy[0].datos_anteriores.hora_programada_entrada,null);
  assert.equal(auditoriaLegacy[0].datos_nuevos.registro.confirmacion_programacion_historica.motivo,confirmacion.motivo);
  log('confirmación explícita del horario antiguo, cálculo y auditoría sin duplicados');
  await fs.mkdir(path.resolve(__dirname,'../backups'),{recursive:true});
  await fs.writeFile(path.resolve(__dirname,'../backups/validacion-sesion.json'),JSON.stringify({baseUrl,tokens,empleadoId,...ids,documentoId,permisoId:permiso,legacyEmpleadoId:rrhh,legacyFecha:'2026-09-18'},null,2),{mode:0o600});
