...
const llamadas = [];
const transacciones = [];
const pool = { query: (sql,params=[]) => { llamadas.push({sql,params}); return queryImpl(sql,params); }, async getConnection(){return {...pool,beginTransaction:async()=>{transacciones.push('begin');},commit:async()=>{transacciones.push('commit');},rollback:async()=>{transacciones.push('rollback');},release(){transacciones.push('release');}};} };
...
test.beforeEach(()=>{llamadas.length=0;transacciones.length=0;});
...
test('registro antiguo sin programación conserva tardanza y horas al editar observaciones',async()=>{
 ...
 assert.throws(()=>reglas.calcularMarcacion({hora_ingreso:'08:30',hora_salida:'17:00',estado:'presente'},null,anterior),/programación original/);
...
test('horario parcial con tolerancia desconocida exige confirmación para cambiar el ingreso o pasar a presente',()=>{
 const anterior={hora_programada_entrada:'08:00:00',hora_programada_salida:'17:00:00',tolerancia_minutos:null,hora_ingreso:'08:17:00',hora_salida:'17:00:00',estado:'tardanza',minutos_tardanza:7,horas_trabajadas:8.72};
 const programacion={hora_entrada:'08:00:00',hora_salida:'17:00:00',tolerancia_minutos:0};
 assert.equal(reglas.calcularMarcacion({estado:'tardanza',hora_ingreso:'08:17',hora_salida:'17:00'},programacion,anterior).minutos_tardanza,7);
 assert.throws(()=>reglas.calcularMarcacion({estado:'presente',hora_ingreso:'08:30',hora_salida:'17:00'},programacion,anterior),/tolerancia/);
 assert.throws(()=>reglas.calcularMarcacion({estado:'presente',hora_ingreso:'08:17',hora_salida:'17:00'},programacion,{...anterior,estado:'justificado'}),/tolerancia/);
});
test('confirmar horario original completa snapshot y recalcula con auditoría en la misma transacción',async()=>{
 const anterior={id:79,empleado_id:10,hora_programada_entrada:null,hora_programada_salida:null,tolerancia_minutos:null,hora_ingreso:'08:17:00',hora_salida:'17:00:00',estado:'tardanza',minutos_tardanza:7,horas_trabajadas:8.72};
 queryImpl=async sql=>{if(sql.startsWith('SELECT id,estado FROM empleados'))return[[{id:10,estado:'activo'}]];if(sql.startsWith('SELECT * FROM asistencias'))return[[anterior]];if(sql.startsWith('UPDATE asistencias'))return[{affectedRows:1}];if(sql.startsWith('INSERT INTO historial_cambios')){assert.ok(!transacciones.includes('commit'));return[{insertId:123}];}throw new Error('No consultar el horario vigente: '+sql);};
 const confirmacion={confirmada:true,hora_entrada:'08:00',hora_salida:'17:00',tolerancia_minutos:10,cruza_medianoche:false,motivo:'Horario original firmado, confirmado por RRHH'};
 const res=response();await asistencia.registrarMarcacion(req({empleado_id:10,fecha:'2026-09-01',hora_ingreso:'08:30',hora_salida:'17:00',estado:'presente',programacion_historica:confirmacion}),res);
 assert.equal(res.statusCode,200);assert.equal(res.body.data.minutos_tardanza,30);assert.equal(res.body.data.horas_trabajadas,8.5);assert.equal(res.body.data.tolerancia_minutos,10);
 const update=llamadas.find(x=>x.sql.startsWith('UPDATE asistencias'));assert.match(update.sql,/hora_programada_entrada=\?/);assert.deepEqual(update.params.slice(7),['08:00:00','17:00:00',10,79]);
 const audit=llamadas.find(x=>x.sql.startsWith('INSERT INTO historial_cambios'));assert.equal(audit.params[2],79);assert.equal(JSON.parse(audit.params[3]).tolerancia_minutos,null);assert.equal(JSON.parse(audit.params[4]).registro.confirmacion_programacion_historica.motivo,confirmacion.motivo);assert.equal(JSON.parse(audit.params[4]).solicitud.programacion_historica.confirmada,true);
 assert.deepEqual(transacciones,['begin','commit','release']);assert.equal(res.locals.auditoria.persistida,true);assert.ok(!llamadas.some(x=>x.sql.includes('FROM horarios')));
});
test('la confirmación explícita valida motivo, tolerancia y turno nocturno; no altera campos ya conocidos',()=>{
 const vacia={id:79,hora_programada_entrada:null,hora_programada_salida:null,tolerancia_minutos:null};
 const original={confirmada:true,hora_entrada:'22:00',hora_salida:'06:00',tolerancia_minutos:0,cruza_medianoche:true,motivo:'Horario firmado'};
 const validada=reglas.confirmarProgramacionHistorica(original,vacia);assert.equal(validada.hora_entrada,'22:00:00');
 assert.equal(reglas.calcularMarcacion({estado:'presente',hora_ingreso:'00:15',hora_salida:'06:00'},validada).minutos_tardanza,135);
 for(const cambios of [{confirmada:false},{motivo:''},{hora_entrada:'25:00'},{tolerancia_minutos:-1},{cruza_medianoche:false},{hora_salida:'22:00'}])assert.throws(()=>reglas.confirmarProgramacionHistorica({...original,...cambios},vacia));
 const parcial={...vacia,hora_programada_entrada:'22:00:00'};
 assert.equal(reglas.confirmarProgramacionHistorica(original,parcial).tolerancia_minutos,0);
 assert.throws(()=>reglas.confirmarProgramacionHistorica({...original,hora_entrada:'21:00'},parcial),/valores históricos/);
 const completa={...parcial,hora_programada_salida:'06:00:00',tolerancia_minutos:0};
 assert.throws(()=>reglas.confirmarProgramacionHistorica(original,completa),/no se puede sobrescribir/);
 assert.throws(()=>reglas.confirmarProgramacionHistorica(original,null),/existente/);
});
test('snapshot completo no permite sobreescritura por API, incluso enviando los mismos datos',async()=>{
 const anterior={id:79,empleado_id:10,hora_programada_entrada:'08:00:00',hora_programada_salida:'17:00:00',tolerancia_minutos:0};
 queryImpl=async sql=>sql.startsWith('SELECT id,estado FROM empleados')?[[{id:10,estado:'activo'}]]:sql.startsWith('SELECT * FROM asistencias')?[[anterior]]:Promise.reject(new Error(sql));
 const res=response();await asistencia.registrarMarcacion(req({empleado_id:10,fecha:'2026-09-01',hora_ingreso:'08:30',hora_salida:'17:00',estado:'presente',programacion_historica:{confirmada:true,hora_entrada:'08:00',hora_salida:'17:00',tolerancia_minutos:0,cruza_medianoche:false,motivo:'Repetir'}}),res);
 assert.equal(res.statusCode,409);assert.ok(!llamadas.some(x=>/^(UPDATE|INSERT)/.test(x.sql)));assert.deepEqual(transacciones,['begin','rollback','release']);
});
test('fallo de auditoría revierte la confirmación y el recálculo; no publica persistida',async()=>{
 const anterior={id:79,empleado_id:10,hora_programada_entrada:null,hora_programada_salida:null,tolerancia_minutos:null};
 queryImpl=async sql=>{if(sql.startsWith('SELECT id,estado FROM empleados'))return[[{id:10,estado:'activo'}]];if(sql.startsWith('SELECT * FROM asistencias'))return[[anterior]];if(sql.startsWith('UPDATE asistencias'))return[{affectedRows:1}];if(sql.startsWith('INSERT INTO historial_cambios'))throw Object.assign(new Error('Fixture auditoría fallida'),{code:'TEST_AUDIT_FAILED'});throw new Error(sql);};
 const res=response();await asistencia.registrarMarcacion(req({empleado_id:10,fecha:'2026-09-01',hora_ingreso:'08:30',hora_salida:'17:00',estado:'presente',programacion_historica:{confirmada:true,hora_entrada:'08:00',hora_salida:'17:00',tolerancia_minutos:0,cruza_medianoche:false,motivo:'Horario firmado'}}),res);
 assert.equal(res.statusCode,500);assert.deepEqual(transacciones,['begin','rollback','release']);assert.equal(res.locals.auditoria,undefined);
});
