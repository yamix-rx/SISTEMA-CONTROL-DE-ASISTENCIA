...
| Vencimiento de convenios independiente de Contratos | Fecha editable en la ficha del practicante, visible en Mi panel y alerta propia a 0–30 días por empresa. Tipo contractual «Convenio de prácticas». | Alta, cambio, eliminación explícita, conservación al omitir el campo y alerta sin contrato, con MySQL; validaciones de fecha, límites y roles. |
| Accesos de menú y puesto separado | Practicantes, Permisos y Generar documentos abren vistas enfocadas; puesto se registra independientemente del cargo. | Navegación activa, filtros y carga de las tres vistas a tres tamaños; conservación y consulta real del puesto. |
| Recursos utilizables sin Internet | Tailwind compilado, iconos, fuentes y exportaciones locales, con versiones y licencias; herramientas de build excluidas del servidor público. | Compilación determinista, siete pruebas de recursos y navegador con caché desactivada y conexiones externas bloqueadas. |
...
| --- | --- | --- |
| Pruebas automatizadas del backend | **100 de 100 correctas; 0 fallidas** | Diez archivos de pruebas, incluidos login, convenios, puesto y repetición de la migración. Las pruebas de archivos utilizan almacenamiento temporal. |
| Pruebas de recursos del frontend | **7 de 7 correctas** | PDF multipágina, Excel con estilos, iconos, fuentes, CSS compilado e integridad de los recursos locales. |
| Recorridos HTTP con MySQL real | **11 comprobaciones completas** | Base separada restaurada, cuentas y registros de validación. |
| Navegador: carga y disposición sin Internet | **51 de 51 vistas correctas** | 17 pantallas/vistas a 1440, 768 y 390 píxeles, con conexiones externas bloqueadas y caché desactivada; cero solicitudes externas. |
...
| Navegador: exportaciones sin Internet | **2 descargas y 5 interacciones correctas** | PDF y Excel de Reportes, cuentas/plantillas y tres documentos PDF con las conexiones externas bloqueadas. |
| Interfaz de puesto y convenio | **7 comprobaciones correctas** | Formularios, alertas, vista de practicantes y Mi panel con API simulada; no se escribieron datos operativos. |
...
Los once recorridos con MySQL abarcaron:
...
11. Puesto y convenio persistidos, eliminación explícita y conservación al omitirlos, alerta sin contratos, filtro exclusivo de practicantes y transición a trabajador conservando el historial de prácticas.
...
La ejecución más reciente añadió las vistas enfocadas de Practicantes, Permisos y Generar documentos. Todos sus estilos, fuentes, iconos y exportaciones se cargaron desde la instancia local. Las bibliotecas se ejecutaron sin CDN; el servidor y MySQL siguieron disponibles. [Recursos locales, versiones, licencias y compilación](RECURSOS_LOCALES.md).
...
- `validacion-offline/resultados.json`: las 51 vistas sin dependencias externas, con sus capturas.
- `validacion-offline/interacciones.json` y su carpeta de reportes: documentos, PDF y Excel descargados con conexiones externas bloqueadas.
- `validacion-ui-practicas/resultados.json`: siete comprobaciones de formularios y alertas con API simulada.
- `validacion-migracion-convenios.json`: respaldo y conservación de las cuatro cuentas y los datos existentes al incorporar puesto y convenio.
- `validacion-instalacion-convenios.json`: instalación nueva con ambas columnas y migración repetible.
- `validacion-recursos-servidor.json`: recursos locales publicados y herramientas de compilación fuera del acceso HTTP.
...
- La operación ya no requiere CDN; sí requiere acceso al servidor de SBSS y a su base de datos. Una instalación inicial de dependencias necesita Internet o una caché preparada.
