(() => {
  ...
  function practiceFields() { const practicante = $('pVinculo').value.startsWith('practicante'); $('horasPracticasField').hidden = !practicante; $('pHorasPracticas').required = practicante; }
  const fields = { pTipoDocumento: 'tipo_documento', pDocumento: 'numero_documento', pNombres: 'nombres', pApellidos: 'apellidos', pNacimiento: 'fecha_nacimiento', pTelefono: 'telefono', pCorreo: 'correo_personal', pDireccion: 'direccion', pCarrera: 'carrera', pInstitucion: 'institucion_educativa', pEmpresa: 'empresa_id', pArea: 'area_id', pCargo: 'cargo_id', pVinculo: 'tipo_vinculo', pEstado: 'estado', pIngreso: 'fecha_ingreso', pFinalizacion: 'fecha_finalizacion', pHorasPracticas: 'horas_totales_asignadas', pObservaciones: 'observaciones_rrhh' };
  function openForm(edit) {
    $('formPersonal').reset(); $('pId').value = edit ? state.ficha.empleado.id : '';
    $('errorPersonal').hidden = true; text('modalTitulo', edit ? 'Editar ficha del personal' : 'Registrar personal');
  ...
  $('formPersonal').addEventListener('submit', async event => {
    ...
    if (!payload.tipo_vinculo.startsWith('practicante')) payload.horas_totales_asignadas = 0;
    try {
