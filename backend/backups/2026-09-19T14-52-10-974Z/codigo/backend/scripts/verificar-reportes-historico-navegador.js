...
(async () => {
  ...
  const output = path.resolve(__dirname, '../backups/validacion-visual/reportes-historico-' + Date.now());
  ...
  try {
    ...
    await fs.writeFile(path.join(output, 'reportes-exportacion.json'), JSON.stringify({ checkedAt: new Date().toISOString(), results, javascript: errors, http }, null, 2));
    }
