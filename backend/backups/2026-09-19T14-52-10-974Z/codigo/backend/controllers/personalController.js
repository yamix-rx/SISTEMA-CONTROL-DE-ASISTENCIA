...
exports.listarColaboradores = async (req, res) => {
  const { empresa_id, area_id, cargo_id, estado, buscar } = req.query;

  try {
    let query = `
      SELECT 
        e.id,
        e.empresa_id, e.area_id, e.cargo_id,
    ...
    if (buscar) {
      query += ` AND (CONCAT(e.nombres, ' ', e.apellidos) LIKE ? OR e.numero_documento LIKE ? OR emp.razon_social LIKE ? OR ar.nombre LIKE ? OR c.nombre LIKE ?)`;
      params.push(...Array(5).fill(`%${buscar}%`));
    }
    ...
    try {
      ...
      const [result] = await connection.query(`
        ...
        (tipo_documento, numero_documento, nombres, apellidos, fecha_nacimiento, telefono, correo_personal,
         ...
         horas_totales_asignadas, horas_completadas, estado, fecha_ingreso, fecha_finalizacion, observaciones_rrhh)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?)
      `, [p.tipo_documento, p.numero_documento, p.nombres, p.apellidos, p.fecha_nacimiento, p.telefono,
        ...
        p.tipo_vinculo, p.horas_totales_asignadas, p.estado, p.fecha_ingreso, p.fecha_finalizacion, p.observaciones_rrhh]);
...
exports.actualizarColaborador = async (req, res) => {
  ...
  try {
    ...
    try {
      await connection.beginTransaction();
      const [rows] = await connection.query('SELECT id FROM empleados WHERE id = ? FOR UPDATE', [id]);
      if (!rows.length) { await connection.rollback(); return res.status(404).json({ ok: false, mensaje: 'Colaborador no encontrado.' }); }
      await connection.query(
        'UPDATE empleados SET tipo_documento=?, numero_documento=?, nombres=?, apellidos=?, fecha_nacimiento=?, telefono=?, correo_personal=?, direccion=?, carrera=?, institucion_educativa=?, empresa_id=?, area_id=?, cargo_id=?, tipo_vinculo=?, horas_totales_asignadas=?, estado=?, fecha_ingreso=?, fecha_finalizacion=?, observaciones_rrhh=? WHERE id=?',
        [p.tipo_documento, p.numero_documento, p.nombres, p.apellidos, p.fecha_nacimiento, p.telefono, p.correo_personal, p.direccion, p.carrera, p.institucion_educativa, p.empresa_id, p.area_id, p.cargo_id, p.tipo_vinculo, p.horas_totales_asignadas, p.estado, p.fecha_ingreso, p.fecha_finalizacion, p.observaciones_rrhh, id]);
      if (p.tipo_vinculo.startsWith('practicante')) {
        await connection.query('INSERT INTO practicante_detalles (empleado_id, horas_meta, estado_completado) VALUES (?, ?, FALSE) ON DUPLICATE KEY UPDATE horas_meta = VALUES(horas_meta)',
          [id, p.horas_totales_asignadas > 0 ? p.horas_totales_asignadas : 320]);
      } else {
        await connection.query('DELETE FROM practicante_detalles WHERE empleado_id = ?', [id]);
      }
      await connection.commit();
