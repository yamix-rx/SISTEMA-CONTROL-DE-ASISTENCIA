(async () => {
  ...
  function openAttendance(item){
    ...
    $('aIngreso').value=item?.hora_ingreso?.slice(0,5)||'';$('aSalida').value=item?.hora_salida?.slice(0,5)||'';$('aEstado').value=item?.estado||'presente';$('aObs').value=item?.observacion||'';$('aAvisoHistorico').classList.toggle('hidden',!(item?.asistencia_id&&!item.hora_programada_entrada));toggleTimes();show('modalAsistencia',true);
  }
