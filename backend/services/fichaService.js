...
async function obtenerFicha(id) {
  const [empleadoRows] = await pool.query(`
    SELECT e.id, e.tipo_documento, e.numero_documento, e.nombres, e.apellidos,
           ...
           e.tipo_vinculo, CASE WHEN LOWER(e.tipo_vinculo) LIKE 'practicante%'
             THEN COALESCE(pd.horas_meta, e.horas_totales_asignadas, 0)
             ELSE COALESCE(e.horas_totales_asignadas, 0) END AS horas_totales_asignadas,
           e.fecha_nacimiento, e.telefono, e.correo_personal, e.direccion, e.carrera, e.institucion_educativa,
           e.fecha_ingreso, e.fecha_finalizacion, e.estado, e.observaciones_rrhh, e.puesto,
  ...
  const porcentajeAvance = horasMeta > 0 ? Math.min(horasRealizadas >= horasMeta ? 100 : 99.9, Math.round(horasRealizadas / horasMeta * 1000) / 10) : 0;
