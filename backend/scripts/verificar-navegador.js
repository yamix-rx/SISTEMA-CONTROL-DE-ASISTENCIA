...
async function onlyLocalNetwork(cdp, origin) {
  const external = [];
  await cdp.send('Network.setCacheDisabled', { cacheDisabled: true });
  await cdp.send('Network.clearBrowserCache');
  cdp.on('Fetch.requestPaused', event => {
    const url = new URL(event.request.url);
    const blocked = ['http:', 'https:'].includes(url.protocol) && url.origin !== origin;
    if (blocked) external.push(url.href);
    cdp.send(blocked ? 'Fetch.failRequest' : 'Fetch.continueRequest', {
      requestId: event.requestId, ...(blocked ? { errorReason: 'InternetDisconnected' } : {})
    }).catch(() => {});
  });
...
async function runSmoke() {
  ...
  const output = path.resolve(__dirname, offline ? '../backups/validacion-offline' : '../backups/validacion-visual');
  await fs.mkdir(output, { recursive: true });
  const target = await (await fetch('http://127.0.0.1:' + debugPort + '/json/new?about:blank', { method: 'PUT' })).json();
  const cdp = await CDP.connect(target.webSocketDebuggerUrl);
  await cdp.send('Page.enable'); await cdp.send('Runtime.enable'); await cdp.send('Network.enable');
  const external = offline ? await onlyLocalNetwork(cdp, origin) : [];
  ...
  const matrix = [
    ...
    ['FichaIndividual.html?vista=practicantes', 'rrhh'], ['Registros.html?vista=permisos', 'rrhh'], ['Documentos.html?vista=generar', 'rrhh'],
  ...
  try {
    for (const width of [1440, 768, 390]) {
      ...
      for (const [file, role] of matrix) {
        ...
        const pageUrl = new URL(file, baseUrl + '/');
        const pageName = pageUrl.pathname.slice(1);
        if (pageName === 'FichaIndividual.html' && config.empleadoId) pageUrl.searchParams.set('empleado_id', config.empleadoId);
        ...
        const view = pageUrl.searchParams.get('vista');
        const filename = pageName.replace(/\.html$/i, '') + (view ? '-' + view : '') + '-' + width + '.png';
        await fs.writeFile(path.join(output, filename), Buffer.from(screenshot.data, 'base64'));
        current.screenshot = filename;
        current.externalRequests = external.slice(externalStart);
        if (view) {
          current.view = await cdp.evaluate('({active:document.querySelector("#sidebarMenu a[aria-current=page]")?.textContent.trim(),vista:new URLSearchParams(location.search).get("vista")})');
        }
        const expectedLabels = { practicantes: 'Practicantes', permisos: 'Permisos', generar: 'Generar documentos' };
        current.ok = current.ready && !current.javascript.length && !current.console.length && !current.http.length && !current.network.length && !current.externalRequests.length && !current.layout.overflow.length && current.layout.documentWidth <= width + 12 && current.layout.pathname.toLowerCase().endsWith('/' + pageName.toLowerCase()) && (!view || current.view.active === expectedLabels[view] && current.view.vista === view);
    ...
    await fs.writeFile(path.join(output, 'resultados.json'), JSON.stringify({ checkedAt: new Date().toISOString(), baseUrl, internetBlocked: offline, externalRequests: external, results }, null, 2));
    cdp.close();
...
module.exports = { CDP, runSmoke, onlyLocalNetwork };
